// components/analytics/customers/RepeatCustomersChart.tsx
"use client";
...
export default function RepeatCustomersChart() {
  ...
  if (isLoading) return <ChartSkeletonHeader title="Repeat vs Returning Breakdown" />;
  if (!data || data.totalCustomers === 0) {
    return <EmptyStateView message="No metrics on cohort retention" />;
  }

  const chartData = [
    { name: "New Customers", value: data.newCustomers, percentage: data.newPercentage },
    { name: "Returning Customers", value: data.returningCustomers, percentage: data.returningPercentage },
  ];
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-6 flex items-center gap-2">
        ...
        <div>
          <h3 className="text-base font-semibold text-slate-900 dark:text-slate-50">Repeat vs Returning</h3>
          <p className="text-xs text-slate-400 dark:text-slate-500">Consumer retention ratios</p>
        </div>
      </div>
      ...
      <div className="h-80 w-full text-xs flex items-center justify-center">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={chartData} cx="50%" cy="45%" innerRadius={60} outerRadius={85} paddingAngle={4} dataKey="value">
