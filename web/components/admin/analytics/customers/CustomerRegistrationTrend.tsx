// components/analytics/customers/CustomerRegistrationTrend.tsx
"use client";
...
export default function CustomerRegistrationTrend() {
  ...
  if (isLoading) return <ChartSkeletonHeader title="Customer Registration Trend" />;
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base font-semibold text-slate-900 dark:text-slate-50">Customer Registration Trend</h3>
            <p className="text-xs text-slate-400 dark:text-slate-500">Inbound velocity variations of target traffic</p>
          </div>
        </div>
      ...
      <div className="h-80 w-full text-xs">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 5, right: 5, left: -15, bottom: 5 }}>
            ...
            <Tooltip
              formatter={(value: any) => [Number(value).toLocaleString("en-IN"), "Acquisitions"]}
              contentStyle={{ background: "#0f172a", borderRadius: "8px", border: "none", color: "#f8fafc" }}
            />
            <Line type="monotone" dataKey="customers" stroke="#0ea5e9" strokeWidth={2.5} activeDot={{ r: 6 }} dot={{ r: 0 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
