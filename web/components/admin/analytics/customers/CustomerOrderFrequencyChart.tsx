// components/analytics/customers/CustomerOrderFrequencyChart.tsx
"use client";
...
export default function CustomerOrderFrequencyChart() {
  ...
  if (isLoading) return <ChartSkeletonHeader title="Order Frequency Distribution" />;
  if (!data || data.length === 0) return <EmptyStateView message="No distribution metrics available" />;
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-6 flex items-center gap-2">
        ...
        <div>
          <h3 className="text-base font-semibold text-slate-900 dark:text-slate-50">Order Frequency Distribution</h3>
          <p className="text-xs text-slate-400 dark:text-slate-500">Customer breakdown by transaction velocity density</p>
        </div>
      </div>
      ...
      <div className="h-80 w-full text-xs">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 5, right: 5, left: -15, bottom: 5 }}>
            ...
            />
            <Bar dataKey="customers" fill="#6366f1" radius={[4, 4, 0, 0]} maxBarSize={45} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
