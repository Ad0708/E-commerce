// components/analytics/products/ProductPerformanceTable.tsx
...
export function ProductPerformanceTable({ listing }: { listing: TableProductItem[] }) {
  return (
      ...
      <table className="w-full text-left text-sm text-slate-500 dark:text-slate-400">
        <thead className="sticky top-0 z-10 bg-slate-50 text-xs font-semibold uppercase text-slate-700 dark:bg-slate-800/80 dark:text-slate-300 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700">
          <tr>
            ...
            <th scope="col" className="px-6 py-3 text-center">Stock Status</th>
          </tr>
        </thead>
            ...
            >
              <td className="whitespace-nowrap px-6 py-4 font-medium text-slate-900 dark:text-slate-100">
                <div className="flex items-center gap-3">
                  <div className="relative h-10 w-10 flex-shrink-0 rounded-lg overflow-hidden border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
                    <Image
                      src={product.image}
                      alt={product.name}
                      fill
                  ...
                  <span className="truncate max-w-[160px] text-sm font-semibold">
                    {product.name}
                  </span>
                </div>
              </td>
...
export function TableSkeletonHeader({ title, description }: { title: string; description: string }) {
  return (
